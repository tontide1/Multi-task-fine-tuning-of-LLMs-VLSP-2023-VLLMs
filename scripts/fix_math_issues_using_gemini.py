...
def math_dollar_positions(text: str) -> list[int]:
    ...
    This prevents false unbalanced-dollar flags on money amounts.
    """
    ignored_positions = {
        m.start()
...
def wrap_matches_outside_math(text: str, pattern: re.Pattern) -> str:
    """
    Wrap regex matches outside $...$ into $...$.
    Does not touch matches already inside math dollars.
    """
    parts = []
    last = 0
...
def deterministic_math_fix(text: str) -> str:
    ...
    - \\frac{a}{b} -> $\\frac{a}{b}$
    - \\sqrt{x} -> $\\sqrt{x}$
    - standalone powers outside math -> $2^{20}$

    Does not try to fix genuinely ambiguous unbalanced dollar cases.
    """
    fixed = text
...
def main():
    ...
    for _, issue in df_issues.iterrows():
        ...
        if rule_fixed_text != original_text:
            # Use rule-fixed text as Gemini input, not raw text.
